...
function CryptoPro(requestUrl, processUrl, completeHandler, stateChangeHandler) {
	this.requestUrl = requestUrl;
	this.processUrl = processUrl;
	this.completeHandler = completeHandler;
	this.stateChangeHandler = stateChangeHandler;
	this.signedObjId = null;
	this.state = "ready";
...
CryptoPro.prototype.makeSign = function(objId, certKey) {
	if (this.state != "ready") {
		this.notifyStateChanged(objId, "not ready");
		this.notifyComplete(objId, "not ready");
	} else {
		this.signedObjId = objId;
		...
		}).always(this.onReceivingComplete);
	}
};

CryptoPro.prototype.onReceivingComplete = function(data, textStatus, jqXHR) {
	...
	if (success) {
		...
		if (contentType.indexOf("text/plain") == 0) {
			...
			this.fail("Receiving data failed: unknown content type '" + contentType + "'");
		}
		if (signFunc) {
			var store = this.getStore();
			try {
				var cert = this.getCertificate(store, this.certKey);
				if (!cert) {
					this.fail("can't get certificate");
				} else {
					var sign = signFunc.call(this, data, cert);
					if (!sign) {
						this.fail("can't sign data");
					} else {
						this.sendSign(sign);
					}
				}
		...
		this.fail(textStatus);
	}
};

CryptoPro.prototype.sendSign = function(sign) {
	this.setState("sending");
	$.ajax({
		context: this,
	    type: "POST",
	    url: this.processUrl,
	    data: {id: this.signedObjId, sign: sign},
	    contentType: "application/x-www-form-urlencoded",
	    beforeSend: function(xhr) {
		    xhr.setRequestHeader('x-requested-with', 'XMLHttpRequest');
	    }
	}).always(this.onSendingComplete);
};

CryptoPro.prototype.onSendingComplete = function(data, textStatus, jqXHR) {
	var success = (textStatus == "success");
	if (!success) {
		this.fail("Sending signature data failed with status '" + textStatus + "'");
	} else {
    	var dt = typeof data;
    	if (dt != "object") {
    		this.fail("Sending signature data failed: unknown response data type '" + dt + "'");
    	} else {
    		if (dt.error) {
    			this.fail("Sending signature data failed: server returned '" + dt.error + "'");
    		} else {
    			this.done();
    		}
    	}
...
CryptoPro.prototype.done = function() {
	this.notifyStateChanged(this.signedObjId, "success");
	this.notifyComplete(this.signedObjId, "");
	this.setReady();
};


CryptoPro.prototype.fail = function(errorMsg) {
	this.notifyStateChanged(this.signedObjId, "fail");
	this.notifyComplete(this.signedObjId, errorMsg);
	this.setReady();
};

CryptoPro.prototype.setReady = function() {
	this.signedObjId = null;
	this.state = "ready";
};

CryptoPro.prototype.setState = function(newState) {
	if (this.state != newState) {
		this.state = newState;
		this.notifyStateChanged(this.signedObjId, this.state);
	}
};

CryptoPro.prototype.notifyStateChanged = function(objId, text) {
	if (this.stateChangeHandler) {
		this.stateChangeHandler(objId, text);	
	}
};

CryptoPro.prototype.notifyComplete = function(objId, errorText) {
	if (this.completeHandler) {
		this.completeHandler(objId, errorText);
	}
};
