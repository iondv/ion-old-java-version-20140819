...
<html>
	<head>
		<title>${title}</title>
		<link href="<c:out value="${Root}" />theme/css/global.css" rel="stylesheet" />
		<link href="<c:out value="${Root}" />theme/css/admin.css" rel="stylesheet" />
		<script src="<c:out value="${Root}" />theme/js/jquery-1.11.0.min.js"></script>
		<link rel="stylesheet" href="<c:out value="${Root}" />theme/js/jquery-ui/themes/base/jquery.ui.all.css" />
		<script src="<c:out value="${Root}" />theme/js/jquery-ui/ui/jquery.ui.core.js" ></script>
		<script src="<c:out value="${Root}" />theme/js/jquery-ui/ui/jquery.ui.widget.js" ></script>
		<script src="<c:out value="${Root}" />theme/js/jquery-ui/ui/jquery.ui.datepicker.js" ></script>			
	</head>
...
<!--[if (gt IE 9)|!(IE)]><!--><body><!--<![endif]-->
    <div id="header">
    	<div class="ribbon">
	    	...
	    	<ul>
				<li>
					<a class="overflowed-text" href="<c:out value="${Root}/user" />">Пользователи</a>
				</li>
				<li>
					<a class="overflowed-text" href="<c:out value="${Root}/auth" />">Роли пользователей</a>
				</li>
				<li>
					<a class="overflowed-text" href="<c:url value="/logout" />">Выйти</a>
				</li>
	    	</ul>
	    	</div>
		...
		<div class="breadcrumbs">	
			<c:forEach items="${breadcrumbs}" var="bc">	
				<img class="arrow" src="<c:out value="${Root}" />theme/img/Arrow1_32.png" />
				<a href="<c:out value="${Root}${bc.url}"/>"><c:out value="${bc.caption}" /></a>
			</c:forEach>
		</div>    
    </div>   
    <div id="content">
